# Audit-ShopDisks Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** PowerShell script that audits disk space on 66 shop computers via WinRM and generates an HTML report with top 50 files and top 20 folders per PC, organized by shop.

**Architecture:** Single script `Audit-ShopDisks.ps1` with 4 phases: connectivity check, parallel remote data collection via `Invoke-Command -AsJob`, result grouping by shop prefix, and HTML report generation with inline CSS.

**Tech Stack:** PowerShell 5.1+, WinRM (Invoke-Command), HTML/CSS inline

---

### Task 1: Script Skeleton - Parameters, Server List, Write-Log

**Files:**
- Create: `Audit-ShopDisks.ps1`

**Step 1: Create script with param block, server list, and Write-Log function**
...
Run: `pwsh -NoProfile -Command "& { . ./Audit-ShopDisks.ps1 -TopFiles 10 }" 2>&1 | head -5`
Expected: Either runs and exits (credential prompt may appear) or shows no parse errors. If on Linux/WSL, just verify syntax: `pwsh -NoProfile -Command "try { [System.Management.Automation.Language.Parser]::ParseFile('$(pwd)/Audit-ShopDisks.ps1', [ref]$null, [ref]$null) | Out-Null; 'SYNTAX OK' } catch { $_ }"`

**Step 3: Commit**

```bash
git add Audit-ShopDisks.ps1
git commit -m "feat(shops): script skeleton with params, server list, Write-Log"
```

---

### Task 2: Connectivity Check Function

**Files:**
- Modify: `Audit-ShopDisks.ps1`

**Step 1: Add Test-ServerConnectivity function after Write-Log**

Insert after the `Write-Log` function closing brace:
...
**Step 2: Verify syntax still valid**

Run: `pwsh -NoProfile -Command "[System.Management.Automation.Language.Parser]::ParseFile('$(pwd)/Audit-ShopDisks.ps1', [ref]\$null, [ref]\$errs); if (\$errs.Count -eq 0) { 'SYNTAX OK' } else { \$errs }"`
Expected: `SYNTAX OK`
...
```bash
git add Audit-ShopDisks.ps1
git commit -m "feat(shops): add Test-ServerConnectivity function"
```

---

### Task 3: Remote Data Collection ScriptBlock

**Files:**
- Modify: `Audit-ShopDisks.ps1`

**Step 1: Add the remote scriptblock variable after the functions section**

Insert after `Test-ServerConnectivity` function, before any main logic:
        ...
        foreach ($disk in $disks) {
            ...
            try {
                $files = Get-ChildItem -Path $driveLetter -Recurse -File -ErrorAction SilentlyContinue |
                    ...
                    Select-Object -First ($TopFiles * 2)  # recolhe mais para depois filtrar global
                $allFiles += $files
            }
            catch {
...
```bash
git add Audit-ShopDisks.ps1
git commit -m "feat(shops): add remote data collection scriptblock"
```

---

### Task 4: HTML Report Generator Function

**Files:**
- Modify: `Audit-ShopDisks.ps1`

**Step 1: Add New-HtmlReport function after the scriptblock**
...
```bash
git add Audit-ShopDisks.ps1
git commit -m "feat(shops): add HTML report generator function"
```

---

### Task 5: Main Orchestration Logic

**Files:**
- Modify: `Audit-ShopDisks.ps1`

**Step 1: Add main execution block at the end of the script**
...
```bash
git add Audit-ShopDisks.ps1
git commit -m "feat(shops): add main orchestration logic (4 phases)"
```
...
**Step 2: Do a full review of the script**

Read `Audit-ShopDisks.ps1` end-to-end and verify:
- All ASCII characters (no accented chars in strings)
- All `${Variable}:` patterns use braces correctly
- No missing closing braces or quotes
- Parameter defaults are sensible
- HTML is well-formed

**Step 3: Run final syntax check**

Run: `pwsh -NoProfile -Command "[System.Management.Automation.Language.Parser]::ParseFile('$(pwd)/Audit-ShopDisks.ps1', [ref]\$null, [ref]\$errs); if (\$errs.Count -eq 0) { 'SYNTAX OK - ' + (Get-Content './Audit-ShopDisks.ps1' | Measure-Object -Line).Lines + ' linhas' } else { \$errs }"`
Expected: `SYNTAX OK - NNN linhas`

**Step 4: Commit everything**

```bash
git add Audit-ShopDisks.ps1 Reports/.gitkeep docs/
git commit -m "feat(shops): complete Audit-ShopDisks v1 - disk audit for Artsana shops"
```
...
- **ASCII only**: All strings in the .ps1 must use ASCII. Use "nao" instead of "nao", "opcao" instead of "opcao", etc. No em-dashes, curly quotes, or accented characters.
- **Variable braces**: When `$var` is followed by `:`, use `${var}:` to prevent PowerShell from interpreting it as a drive/scope reference.
- **Testing**: This script targets Windows machines via WinRM. Syntax can be verified on Linux/WSL with `pwsh`, but functional testing requires the actual shop network.
- **Report output**: HTML file saved to `./Reports/AuditDisk_YYYY-MM-DD_HHmmss.html`
